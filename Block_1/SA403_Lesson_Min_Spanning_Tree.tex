...
\begin{center}
 \begin{tikzpicture}[every node/.style={circle, fill=green,minimum width=5mm,draw,shading=axis,top color=green, bottom color=green!50!black}]

 \node (a) at (.75,0){Jim};
 \node (b) at (-1.5,0){Dwight};

 \node (c) at (-4,0){Phyl};
 \node (d) at (-4.75,-1.5){Andy};
 \node (e) at (-5.5,0){Stan};
 \node (f) at (4.5,4) {Ang};
 \node (g) at (-4,4.75) {Creed};
 \node (h) at (0,4.75) {Mere};
 \node (i) at (3,4.75) {Oscar};
 \node (j) at (4.5,5.5) {Kev};
 \node (k) at (4.5,0) {Pam};
 \node (l) at (0.5,-4) {Mike};
...
\begin{scope}[on background layer]
  ...
  \foreach \alpha in {b}%
  {%
  \foreach \alphb in {h,f,c,j,g}%
  {%
   \draw (\alpha) -- (\alphb);%
  }}
  \foreach \alpha in {i}%
  {%
  \foreach \alphb in {f,j,h,k}%
  {%
   \draw (\alpha) -- (\alphb);%
  }}
  \foreach \alpha in {k}%
  {%
  \foreach \alphb in {f,l,j}%
  {%
   \draw (\alpha) -- (\alphb);%
  }}
  \foreach \alpha in {g}%
  {%
  \foreach \alphb in {h,c,e}%
  {%
   \draw (\alpha) -- (\alphb);%
  }}
  \foreach \alpha in {d}%
  {%
  \foreach \alphb in {l,c,e,b}%
  {%
   \draw (\alpha) -- (\alphb);%
  }}
  \foreach \alpha in {e}%
